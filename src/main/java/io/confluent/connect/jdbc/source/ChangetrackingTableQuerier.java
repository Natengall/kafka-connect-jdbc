...
public class ChangetrackingTableQuerier extends TableQuerier {
  ...
  public ChangetrackingTableQuerier(DatabaseDialect dialect, QueryMode mode, String name, String topicPrefix,
  ...
  protected void createPreparedStatement(Connection db) throws SQLException {
    // Default when unspecified uses an auto incrementing column
    if (incrementingColumn != null && incrementingColumn.isEmpty()) {
      incrementingColumn = JdbcUtils.getAutoincrementColumn(db, this.config.getString(JdbcSourceTaskConfig.SCHEMA_PATTERN_CONFIG), tableId.tableName());
    }

    String quoteString = JdbcUtils.getIdentifierQuoteString(db);

    StringBuilder builder = new StringBuilder(getTransactionLevelString(config.getString(JdbcSourceConnectorConfig.TRANSACTION_ISOLATION_LEVEL_CONFIG)));
    ...
    log.info("{} prepared SQL query: {}", this, connectorSql);
  ...
  public SourceRecord extractRecord() throws SQLException {
    ...
    offset = extractOffset(schemaMapping.schema(), record);
    final String topic;
    final Map<String, String> partition;
    switch (mode) {
      case TABLE:
        partition = Collections.singletonMap(JdbcSourceConnectorConstants.TABLE_NAME_KEY, tableId.tableName());
        topic = topicPrefix + tableId.tableName();
        break;
      case QUERY:
    ...
    String partitionColumn = config.getString(JdbcSourceConnectorConfig.PARTITION_COLUMN_NAME_CONFIG);
    String keyColumn = config.getString(JdbcSourceConnectorConfig.KEY_COLUMN_NAME_CONFIG);
  ...
  private String getDynamicConnectorQuery() {
    return "IF OBJECT_ID('tempdb..#kc_primary_keys') IS NOT NULL " +
      ...
      "SET @table_name = '" + tableId.tableName() + "'; " +
      ...
      "SELECT 'SELECT * FROM ( SELECT ' + @columns + ' FROM CHANGETABLE (CHANGES ' + @db_name + '.dbo.' + @table_name + ', ?) AS CT LEFT OUTER JOIN ' + @db_name + '.dbo.' + @table_name + ' AS P ON ' + @pk_equals + '" + (this.operations == null || this.operations == "" ? "" : " WHERE SYS_CHANGE_OPERATION IN (" + this.operations + ") ") + ") AS RESULTS '";
  }
}